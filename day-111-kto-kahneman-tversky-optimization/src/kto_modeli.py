"""
KTO Dil Modeli Mimarisi (Day 111).
Causal Transformer otoregresif mimarisi ve tekil diziler için log-olasılık çıkarım motoru.
"""
...
from .kto_kaybi import hesapla_dizi_logprob
...
class KTODilModeli(nn.Module):
    """KTO Eğitimi için Otoregresif Transformer Dil Modeli."""
    ...
    def logprob_hesapla(self, input_ids: torch.Tensor, maske: torch.Tensor) -> torch.Tensor:
        """Verilen dizi ve maske için toplam yanıt log-olasılığını [B] döndürür."""
        logits = self.forward(input_ids)
        return hesapla_dizi_logprob(logits, input_ids, maske)
